...
const LetterHex: FunctionComponent<LetterHexProps> = ({scalar, center, letter}) => {
    ...
    <div className="hex-grand-wrapper">
      <svg height={heightCoord} width={width}>
        <polygon
          ...
          points={`0,${heightCoord/2} ${width/4},0 ${3 * width /4},0 ${width},${heightCoord/2} ${3 * width /4},${heightCoord} ${1 * width /4},${heightCoord}`} stroke="black"
          onClick={(e)=>{selectLetter(e, letter)}}
        >
        </polygon>
      </svg>
      <p>{letter}</p>
